...
// --- TYPES (TypeScript Errors Hatane Ke Liye) ---
interface Tier {
  quantity: string;
  price: string;
}
...
interface DiscountRule {
  ...
  tiers: string; // JSON String stored in DB
}

// --- 1. BACKEND: LOADER (READ) ---
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
  const { session } = await shopify(context).authenticate.admin(request);
  
  // Cloudflare DB Context agar required ho, warna direct db import use karein
...
// --- 2. BACKEND: ACTION (CREATE, UPDATE, DELETE) ---
export const action = async ({ request, context }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (!productId || !tiersString) {
      return json({ success: false, message: "Missing required fields" });
    }

    // --- CREATE ---
    if (actionType === "create") {
      // Logic: Check karo agar product pehle se exist karta hai
      const existing = await db(context.cloudflare.env.DATABASE_URL).volumeDiscount.findFirst({
        where: { shop, productId }
      });
      
      if (existing) {
        return json({ success: false, message: "Rule already exists for this product. Please Edit instead." });
      }
      ...
      return json({ success: true, message: "Volume Discount Created!" });
    }
    ...
    if (actionType === "update") {
      ...
      await db(context.cloudflare.env.DATABASE_URL).volumeDiscount.update({
        where: { id },
        data: { productTitle, tiers: tiersString } // Product ID change nahi karte update me usually
      });
      return json({ success: true, message: "Volume Discount Updated!" });
    }

    return null;
  } catch (error: any) {
    console.error(error);
    return json({ success: false, message: "Server Error: " + error.message });
  }
};
